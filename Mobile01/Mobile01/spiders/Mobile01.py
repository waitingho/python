#//*[@id="maincontent"]/div[6]/table/tbody/tr[1]/td[1]/span/a
...
class Mobile01(scrapy.Spider):
	...
	def start_requests(self):
		...
		for i in range(1,int(self.p)+1):
			str_idx=''+('%s'%i)
			tmp='https://www.mobile01.com/forumtopic.php?c=%s&p=%s' % (c,str_idx)#to do
	...
	def parse(self,response):
		...
		for i in range(1,count+1):
			...
			titeList  = response.xpath(str_xpath).extract()
			strTitle  = ''.join(titeList)
